# δ-as-agent receipt — zeroth-pilot wave 2026-05-15

## 1. WAVE OUTCOME

**REVISE** — methodology and pipeline are sound; OpenCap Lab Validation acquisition is blocked at SimTK's account-required download endpoint (operator credentials needed).

## 2. PER-ISSUE SUMMARY

| # | Rounds | β verdict | Final SHA on main | Key artifacts | Debt |
|---|---|---|---|---|---|
| 5 | 1 | APPROVE | fbcfbaf | `data/external/opencap-lab-validation.md` (fully populated, OpenCap Lab Validation selected, Apache 2.0); `.gitignore` extended; `.cdd/unreleased/5/*` | SimTK login blocks acquisition; speed-graduation partial; row-level inventory pending download |
| 6 | 2 | APPROVE (after F1 fix: pyc files) | 8997f6f | `notebooks/existing-data-processing.ipynb` (3 figures inline, executes end-to-end); `scripts/{io_opencap,segmentation,features,comparison,build_notebook}.py`; `requirements.txt` (pinned); `notebooks/README.md`; `analysis/feature-summary-zeroth-pilot.md`; `.cdd/unreleased/6/*` | Real-data IK/marker pairing deferred; `/opt/gait-data/` hard-coded; hip ab/ad-duction features absent; smoke-vs-real disclaimer must propagate |
| 7 | 1 | APPROVE | c739b98 | `reports/field-report-01-existing-data-zeroth-pilot.md` (rewritten — 9 protocol outputs covered, 6-condition falsification table, 3 hypotheses, REVISE decision); `PROJECT.md` updated (Current Stage + Realization 04); `.cdd/unreleased/7/*` | Real-data hypothesis evaluation pending; protocol revision recommended but out of cycle scope per non-goals |

## 3. WAVE-CLOSEOUT.MD

The full body of `.cdd/waves/zeroth-pilot-2026-05-15/wave-closeout.md` is reproduced inline below for receipt completeness; the canonical copy lives at that path.
...
- Cycle branches `cycle/5`, `cycle/6`, `cycle/7` all merged into `main` and deleted from `origin`.
- No tags created (tag/release is operator-gated per wave manifest; not exercised).
- Pre-existing `origin/claude/add-support-paths-readme-t5ZW3` branch is unrelated to this wave and was left untouched.

## 5. ESCALATIONS

1. **SimTK access mechanism (load-bearing).** OpenCap Lab Validation is Apache 2.0 permissive but SimTK file downloads require an authenticated user account. The wave's "Download public OpenCap Lab Validation data" permission did not include operator-supplied credentials. The OpenCap public S3 bucket denies listing; the OpenCap Python client requires an API token. All three access paths blocked. Documented acquisition procedure for an operator-with-credentials is committed at `data/external/opencap-lab-validation.md §Acquisition procedure`.

2. **Wave-manifest escalation rule shape.** The manifest's rule "If the candidate dataset has a non-permissive license, escalate" did not cover the actual failure mode (permissive license + gated access). Wave-manifest convention should be broadened to "non-permissive license OR access-mechanism gate that requires unsupplied credentials."

3. **Multi-sub parent auto-close assumption.** Wave manifest stated "#4 auto-closes when all subs close" — false. GitHub auto-close fires per merge-commit `Closes` directive only. γ closed #4 manually.

No AC reinterpretations were required. No license escalation fired (Apache 2.0 is unambiguously permissive). β never exceeded 3 fix-rounds (max was 1, on #6).

## 6. KNOWN GAPS (aggregated)

From #5 self-coherence §Debt:
- SimTK download blocked on operator credentials.
- Speed-graduation partial (dataset has 2 conditions at self-selected speed, not graded speeds).
- Row-level walking-trial inventory blocked on download.

From #6 self-coherence §Debt:
- Real-data OpenCap-vs-reference pairing logic NOT IMPLEMENTED (requires archive to verify file-naming convention).
- Smoke vs real disclaimer must propagate to Sub C.
- Shape PCs, asymmetry waveform diffs, condition-response deltas deferred to Sub C.
- `opensim` / `nimblephysics` not installed (first-pass kinematics don't need it; second-pass dynamics would).
- Aggregate summary at `analysis/feature-summary-zeroth-pilot.md` is auto-generated.

From #7 self-coherence §Debt:
- Real-data hypothesis evaluation deferred to post-acquisition cycle.
- Protocol revision authoring (Access mechanism subsection) intentionally out of cycle scope per issue non-goals.
- Hip ab/ad-duction features absent from first-pass extraction.
- Smoke disclaimer salience: real-data re-run must refresh §Trial Inventory, §Segmentation Status, §Feature Extraction Status, §OpenCap vs Reference Comparison rows.

Wave-level synthesis: 4 cdd-*-gap patterns (1 protocol-gap, 1 skill-gap candidate, 2 tooling-gap) identified across cycles. Patches 3 (parent auto-close) and 4 (`__pycache__`) landed during the wave; patches 1 (access mechanism) and 2 (falsification preamble) are recommended for follow-up cycles per the wave close-out.
