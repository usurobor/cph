...
6. **Docs** — covered by Step B's status-surface realignment:
...
7. **Self-coherence** — this file. Written incrementally per §2.5 with one section per commit (commits `2392ef3` §Gap, `822ba6e` §Skills, `bb0ee76` §ACs, `c13eec3` §Self-check, `1c7fed0` §Debt, and the next commit will land §CDD-Trace).
...
| 17 | Pre-review gate run | (next: §"Pre-review gate" section) |
| 18 | Review-readiness signal | (next: §"Review-readiness" section) |
