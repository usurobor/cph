...
> **⚠ Stream-A scaffolding caveat (applies to all entries 0.3.0 onward).** Empirical-state claims and per-version verdicts in the entries below were recorded during Stream-A (engineering) cycles that did not have the OpenCap data archive (`/opt/gait-data/opencap-lab-validation/extracted/`) mounted in the dispatch container. The code shipped in each version is real and executable; the numerical claims in each entry are scaffolding pending Stream-B (science) validation. The 0.1.0 / 0.2.0 entries pre-date the Stream-A vs Stream-B split and were docs/process refactors with no empirical claims to validate.

## 0.4.1 — R3 bilateral extension; H3 indeterminate on inferred-bilateral surface (2026-05-20)

**Where we were.** After cph#27 (R3 R-side partial GO; 7 of 25 features BH-significant at q<0.05; H3 structurally non-testable on R-side-only surface; condition 3 not testable on this archive) and cph#28 (R1 → GO with bounded scope; 57 inferred-partial L cycles via half-stride contralateral inference; condition 3 surface lifted to "evaluable on inferred-bilateral"), the project had a bilateral surface available but no inferential statement about whether L-vs-R lr-diff features actually responded as the asymmetric phase-coupling hypothesis (H3) predicts on the inferred surface. cph#31 (R4 full falsification, merged 2026-05-20) deferred the condition 3 substantive verdict to cph#30 explicitly. cph#30's open question: does the post-cph#28 inferred-bilateral surface support H3 (asymmetric phase-coupling) with the path (a) honesty caveat applied, and does it separate "consistent with real asymmetric coordination" from "consistent with the partial-clip geometric bias predicted by cph#28's matched-duration L-cycle recovery"?

**What this version unblocked.** The bilateral lr-diff aggregate analysis exists. cph#30 runs `analysis/r3_bilateral_tests.py` (new sibling script to `analysis/r3_subject_aggregate_tests.py`; the cph#27 R-side script is unchanged) against the post-cph#28 inferred-bilateral feature table (60 R measured + 57 L inferred-partial cycles; 57 paired (subject, trial_id, condition, cycle_number) rows; 10 subjects × 2 conditions × 25 lr_diff features = 50 paired Wilcoxon tests). **21 of 50 (feature × condition) lr-diff tests cleared BH at q<0.05, all 21 in the positive direction (R > L).** Every joint range feature (hip flexion, hip adduction, knee, ankle), every trunk range feature (lumbar bending / rotation / extension), and every pelvis range feature (tilt / list / rotation) cleared BH in both conditions with r_rb = +1.0 (every subject in agreement). The cleanest H3 probe (coordination-lag lr_diff features — timing-based, not biased by amplitude truncation) returned BH-non-significant null in both conditions. The cross-condition shift test (paired Wilcoxon on lr_diff under trunk-sway minus lr_diff under natural per subject, n=10) found *no* feature clearing BH q<0.05 after correction.

**The mechanistic reading (with path (a) caveat applied throughout).** The systematic positive-direction lr-diff pattern is **consistent with** the partial-clip geometric bias predicted by cph#28's matched-duration L-cycle recovery (L cycles cover 0.80–0.94 of the L stride; missing terminal swing biases L range features systematically downward) AND **consistent with** a real asymmetric coordination signature in the underlying L-side motion that the inferred-bilateral surface happens to recover. The two surfaces — measurement of asymmetric coordination versus inference-artifact of the partial-clip geometry — **cannot be distinguished on the inferred-bilateral surface alone.** The coordination-lag null reading is *consistent with* "the range-feature BH-significance is partial-clip artifact" but also *consistent with* "real asymmetric coordination is amplitude-coded, not timing-coded." **H3 verdict: indeterminate.** **Falsification condition 3: evaluable; indeterminate; bounded scope; non-contributory to the threshold tally** (same threshold treatment as "not testable" per `docs/concepts/support-path.md` §Falsification "Empirical-data prerequisite"). cph#31's wave-level "construct survives subject to caveats" verdict carries through unchanged.

**What is now testable on the inferred-bilateral surface.**

- **Bilateral lr-diff aggregate** — 50 (feature × condition) paired Wilcoxon tests at subject-level n=10; BH-FDR at q=0.05; rank-biserial r_rb + percentile bootstrap 95% CI (B=10,000, seed=20260520). Reproducible via [`analysis/r3_bilateral_tests.py`](analysis/r3_bilateral_tests.py).
- **Cross-condition lr-diff shift** — paired Wilcoxon on (lr-diff under trunk-sway − lr-diff under natural) per subject per feature. No BH-significant shifts found, consistent with the partial-clip geometric bias dominating over condition-driven asymmetric coordination response on the inferred surface.
- **Falsification condition 3** — indeterminate, bounded scope, non-contributory to threshold tally; cph#31 R4 wave-level verdict preserved unchanged at 0 of 6 triggered.

**What is still bounded / blocked.**

- **H3 (asymmetric phase-coupling between sides)** — indeterminate on the inferred-bilateral surface. Path (a) honesty caveat blocks the substantive verdict: the BH-significant positive-direction lr-diff pattern on every range feature is geometrically expected from cph#28's partial-clip rule (no real asymmetric coordination needed to produce it), and is also consistent with a real asymmetric coordination signature; the inference layer cannot be separated from real signal on this surface.
- **Path (b) (measured-bilateral upgrade)** — operator-side OpenSim IK rerun on the reachable TRC files at `/opt/gait-data/opencap-lab-validation/extracted/` with extended trial windows would produce *measured* L HS times and full-coverage L cycles, replacing the inference layer. Once measured-bilateral data exists, re-run `analysis/r3_bilateral_tests.py` against the new feature table to substitute the indeterminate H3 verdict for a decisive one. Out of cph#30 scope; requires operator-side OpenSim install.
- **H1 / H2 verdict shifts** — none. cph#27 R-side verdicts preserved unchanged. The bilateral lens adds one new sub-finding (peak_knee_flexion_phase lr-diff BH-significant under walking only, flagged as likely sub-sample noise) and one framing clarification (trunk / pelvis range lr_diff is *shared-signal lr_diff* — one trace per trial windowed against two different cycle slices — not coordination lr_diff; reported but flagged as not adding bilateral evidence for H2).
- **Within-subject cycle-level repeatability** — n=3 R + 1–3 L cycles per (subject, condition) cell. Carried as cph#27 §Debt D2.
- **First-pass PCA / clustering** — held for R6. Carried as cph#27 §Debt D3.
- **R5 (friend pre-pilot)** — unblocked at the gate level (R3 closes on both R-side at partial GO and bilateral at indeterminate-on-inferred surface; R4 survives). The capture-protocol-revision step (≥3 s trial length so future captures do not recur the partial-clip geometric bias) is the remaining R5 prerequisite. R5 has not transitioned to ACTIVE.
- **R6 (AI classification)** — still blocked. R3-bilateral indeterminate does not unblock R6; the confound-check sweep is a separate gate.

**The new question.** With R3 bilateral indeterminate on the inferred-bilateral surface and R4 wave-level "survives subject to caveats" preserved, does the path (b) measured-bilateral upgrade (operator-side OpenSim IK rerun) produce the decisive H3 verdict, or do the cph#30 indeterminate findings reproduce on a measured-bilateral surface (which would itself be a substantive finding — the construct's bilateral asymmetric coordination signature would be neither supported nor refuted at the data depth the existing archive can produce, and a friend pre-pilot capture with ≥3 s trial length would become the next gate)? Owner: a separate cycle for the path (b) upgrade; the R5 protocol-revision cycle is its alternative.

### Changed (file-level)

- `analysis/r3_bilateral_tests.py` — new single-purpose sibling script. Sibling to `analysis/r3_subject_aggregate_tests.py` (cph#27 R-side, unchanged). Reads `$GAIT_DATA_ROOT/cph-features/features-zeroth-pilot.csv` (read-only; not committed). Computes per-(subject, trial_id, condition, cycle_number) lr-diff rows via a local mirror of `scripts.features.lr_asymmetry` (the upstream `scripts/features.py` is source-of-truth code α does not modify per cph#30 dispatch). Aggregates per (subject, condition) by median across cycles; runs paired Wilcoxon signed-rank tests on lr-diff vs 0 per (feature × condition); computes rank-biserial r_rb + percentile bootstrap 95% CI (B=10,000, seed=20260520); BH-FDR at q=0.05 across all 50 feature × condition tests; cross-condition shift tests on (lr-diff under TS − lr-diff under natural). Surrogate mode (deterministic synthetic feature table calibrated to cph#27 §AC1a R-side medians + cph#28 §"cph#28 L-side recovery" L-side cycle counts) fires automatically when the persisted CSV is absent and is documented in the field report as harness-validation only.
- [`reports/field-report-03-construct-evaluation.md`](reports/field-report-03-construct-evaluation.md) — appended new §"R3 bilateral extension (cph#30, 2026-05-20)" section. Carries the 60-row lr-diff frame inventory, the 20×25 (subject, condition, lr_diff feature) median aggregate table, the 50-row paired-test table (with BH-significance markers), the cross-condition shift table, the H3 evidence summary with path (a) caveat, the falsification condition 3 verdict (evaluable; indeterminate; bounded scope), the H1 / H2 verdict-shift analysis (no change vs cph#27), the decision (R3 bilateral = indeterminate), next gates, and the surrogate-vs-canonical substitution protocol for β. cph#27 R-side sections preserved verbatim. New Appendix D documents reproducibility for the bilateral script; new Appendix E carries cph#30 receipt.
- [`PROJECT.md`](PROJECT.md), [`ROADMAP.md`](ROADMAP.md) — realigned to R3 bilateral indeterminate / H3 indeterminate / condition 3 indeterminate reading. PROJECT.md §"Current empirical decision" / §"Current blocker" / §"Next action" / §"Open issues" / §"Last field report" all updated; ROADMAP.md §"Current state" / §"Phase R3" / §"Phase R4" / §"Phase R5" all updated. R4 wave-level "survives" verdict preserved; R3 phase gate status now reads "Partial GO on R-side (cph#27); indeterminate on bilateral inferred-bilateral surface (cph#30)."

### Decision

R3 bilateral closes at **indeterminate on the inferred-bilateral surface (path (a))**; cph#27 R3 R-side partial GO stands. R4 wave-level "construct survives subject to anchor caveats" verdict carries through unchanged (condition 3 indeterminate is non-contributory to threshold tally; tally stays at 0 of 6 triggered). The hypothesis is neither validated nor refuted; the project has moved from "does the bilateral lr-diff aggregate support H3 on the inferred-bilateral surface?" to "is the path (b) measured-bilateral upgrade required to separate real asymmetric coordination from the partial-clip geometric bias, or does the R5 protocol-revision cycle (with ≥3 s trial length) deliver measured-bilateral data on a future archive?"

### Next gate

Path (b) measured-bilateral upgrade (operator-side OpenSim IK rerun on `/opt/gait-data/opencap-lab-validation/extracted/` TRC files with extended trial windows); or, alternatively, the R5 protocol-revision cycle to specify ≥3 s minimum trial length so future captures do not recur the partial-clip geometric bias. Either route produces measured-bilateral data; cph#30's `analysis/r3_bilateral_tests.py` re-runs against the new feature table to substitute the indeterminate H3 verdict for a decisive one.
